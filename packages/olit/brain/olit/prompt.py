...
def seed_dataset_block(dataset_id):
    """ADDED: olit can be opened on a dataset, which loom has no equivalent for.

    Galaxy mounts olit as a visualization plugin, so the user may arrive with one already
    ...
    return (
        f"## Starting dataset\n\n"
        f"The user opened Olit on dataset `{dataset_id}`. Take it as the one they mean when "
