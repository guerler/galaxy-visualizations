...
@dataclass
class BarAggregateShell:
    """Bar Diagram shell implementation for aggregated data."""

    # ShellType interface implementation
    analysis: Optional[AnalysisType] = None
    name: str = "Bar Diagram"
    description: Optional[str] = None
    signatures: List[List[FieldType]] = field(
        default_factory=lambda: [["nominal", "quantitative"], ["ordinal", "quantitative"]]
    )
    required: EncodingMapType = field(
        default_factory=lambda: {"x": {"type": "nominal"}, "y": {"aggregate": True, "type": "quantitative"}}
    )
    ...
    def compile(
        ...
        encoding: Dict[str, Any] = {
            ...
            "y": {"aggregate": "mean", "field": params.get("y", ""), "type": "quantitative"},
        }
